...
def load_big_sentiment_models():
    finbert = BertForSequenceClassification.from_pretrained('yiyanghkust/finbert-tone',num_labels=3)
    tokenizer = BertTokenizer.from_pretrained('yiyanghkust/finbert-tone')

    sent_pipe = pipeline("sentiment-analysis", model=finbert, tokenizer=tokenizer)
    return sent_pipe
...
def download_from_youtube(url, task_id, user_name="youtube_down"):
    if "http" not in url or "youtube" not in url or "www" not in url or "com" not in url:
    ...
    try:
        ...
        try:   
            ...
            print('setting proxy to download...')
            proxy = {
                'https': 'https://118.69.111.51:3128' # Replace with your HTTPS proxy server and port
            }
            ydl_opts = {
                        'proxy': proxy['https'],
                        'geo_bypass': True,
            ...
            with YoutubeDL(ydl_opts) as ydl:
                ...
                title = title.replace(" ","_")
            raw_audio_name = clean_text(title)
            ydl_opts['outtmpl'] = f'temp/{user_name}/{task_id}/{task_id}_{raw_audio_name}'
            with YoutubeDL(ydl_opts) as ydl:
                ydl.download(url)
